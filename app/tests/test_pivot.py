...
def test_analyze_text_without_chiastic_pattern(pivot_client):
    headers = {"X-API-Key": "test-key"}
    text = "The LORD is my shepherd I shall not want He maketh me to lie down in green pastures" # 18 words, no repeats
    response = pivot_client.post(
        "/analyze_text",
        headers=headers,
    ...
    # Golden Ratio check: 18 / 1.618 ~= 11.12 -> rounded to 11
    golden_point = points_by_detector["golden"]
    assert golden_point["position"] == 11
    assert golden_point["score"] == 1.0
